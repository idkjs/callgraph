...
class virtual_functions_json_parser (callee_json_filepath:string) = object(self)
  ...
  method get_redeclared_method (child_record_filepath:string)
			      (child_record_name:string) 
			      (redeclared_method_sign:string) : (Callgraph_t.file * Callgraph_t.fct_def) option = 
    ...
    (match child_class with
	  ...
	  let redeclared_method : (Callgraph_t.file * Callgraph_t.fct_def) option = 
		...
		try
		  (
		    let redeclared_method : Callgraph_t.fct_def = 
		      List.find
  			(
  			  fun (f:Callgraph_t.fct_def) -> String.compare redeclared_method_sign f.sign == 0
			)
			fcts
		    in
  ...
  method add_redefined_methods_to_virtual_declared_method (fct:Callgraph_t.fct_decl) (file:Callgraph_t.file) =
  ...
  (* TODO: If the redefined virtual method is in fact located in another file as the virtual method, *)
  (* then add an external callee to the redefined method. *)
  method add_redefined_methods_to_virtual_defined_method (fct:Callgraph_t.fct_def) (file:Callgraph_t.file) =
            ...
            (fun (all_redefined_methods:(Callgraph_t.file * Callgraph_t.fct_def) list) (record:Callgraph_t.record) -> 
             ...
             (match record.inherited with
			  ...
			  (
			    ...
			    (*TBC Get redefined_method from redeclared method *)
			    redefined_method :: red_methods
			  )
                      )
                     )
