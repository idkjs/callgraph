...
(* WARNING: this backend has no added value any more because we always navigate through the declaration now *)
(* So we will soon remove this backend.                                       *)
(******************************************************************************)
		...
		List.map
			...
			| Some extcallees ->
			  (
					...
					| "unknownExtFctDecl" ->
					  (
					    ...
					    (Printf.printf "Try to look for symbol \"%s\" in the root directory \"%s\"...\n" f.sign rootdir_fullpath;
						 ...
						 in
						 Printf.printf "NOT_FOUND extcallee: sign=\"%s\", decl=%s, mangled=%s\n" f.sign f.decl f.mangled;
						 edited_extcallee
					       )
					     )
					    )
				...
				edited_extcallees
			    in
			    let extcallees : Callers_t.extfctdecl list option =
			      (
