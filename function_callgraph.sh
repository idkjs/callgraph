...
set -x
# author: Hugues Balp
# example: function_callgraph.native callees "main" "int main()" `pwd`/test_dummy.c
...
then
    files=$1
    function_callgraph.native "${direction}" "${caller_jsonfile_absolute_path}" "${caller_id}" "${caller_sign}" $files    
else    
    callee_jsonfile_absolute_path=$1
    callee_id=$2
    callee_sign=$3
    files=$4
    function_callgraph.native "${direction}" "${caller_jsonfile_absolute_path}" "${caller_id}" "${caller_sign}" "${callee_jsonfile_absolute_path}" "${callee_id}" "${callee_sign}"  $files
fi
if [ $? -ne 0 ]; then
    echo "EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"
    echo "ERROR in function_callgraph.native ${src_dirname}/${src_filename} ${defined_symbols}. Stop here !"
    echo "EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"
    exit -1
fi
