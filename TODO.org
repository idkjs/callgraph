...
** share common functions between ocaml programs like [1/1]
   - [X] read_json_file
* TODO add new features [8/12]                              :backlog:callers:
  ...
  - [-] support for _calls between classes_ [1/2]
    - [X] at backend level
    ...
    - [-] at model level [2/3]
      - [X] add a calls reference in class type of callgraph.atd
        inherit depends;
      - [-] fills automatically the calls reference with the callers' extract_fcg backend [2/3]
        - [X] add method extract_fcg.record_add_calls
        - [X] call this method when parsing fct def and callees
        - [ ] call this method when parsing fct decl and callers
